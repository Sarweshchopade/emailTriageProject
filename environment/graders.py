...
class EmailGrader:
    ...
    def score_partial(self, emails: List[Email], actions: List[EmailAction]) -> float:
        if not self.expected:
            return 1.0 - self.STRICT_SCORE_EPSILON
        ...
        raw_score = achieved / total_weight
        if raw_score >= 1.0:
            return round(1.0 - self.STRICT_SCORE_EPSILON, 4)
        return round(raw_score, 4)

    def final_score(self, actions: List[EmailAction]) -> float:
        raw_score = self.score_partial([], actions)
        return self._strict_final_score(raw_score)
    ...
    def _strict_final_score(self, score: float) -> float:
        if score <= 0.0:
            return round(self.STRICT_SCORE_EPSILON, 4)
        if score >= 1.0:
            return round(1.0 - self.STRICT_SCORE_EPSILON, 4)
...
class HardTaskGrader(EmailGrader):
    ...
    def final_score(self, actions: List[EmailAction]) -> float:
        return self._strict_final_score(self.score_partial([], actions))
